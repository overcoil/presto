        ...
        implements ConnectorPageSourceProvider
{
    ...
    {
        DeltaSplit split = (DeltaSplit) connectorSplit;
        DeltaTableLayoutHandle deltaLayout = (DeltaTableLayoutHandle) layout;
        DeltaTableHandle table = deltaLayout.getTable();

        List<DeltaColumnHandle> regularColumns = columns.stream()
                .map(DeltaColumnHandle.class::cast)
                .collect(toImmutableList());

        HdfsContext hdfsContext = new HdfsContext(session, table.getSchemaName(), table.getTableName());
        // TODO: no pruning of partitions yet; thus, we ignore columns and splitContext
            ...
            FileFormatDataSourceStats fileFormatDataSourceStats)
    {
        ...
        try {
            ...
            // Mapping from Delta field ID to Parquet fields.
            ...
            // WAS: support subfield pushdown
            // skip subfield for now
            ...
            TupleDomain<ColumnDescriptor> parquetTupleDomain = getParquetTupleDomain(descriptorsByPath, TupleDomain.none());
            ...
            for (int columnIndex = 0; columnIndex < regularColumns.size(); columnIndex++) {
                DeltaColumnHandle column = regularColumns.get(columnIndex);
