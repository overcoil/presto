...
import com.facebook.presto.deltalake.util.PageListBuilder;
...
import static com.facebook.presto.deltalake.DeltaLakeErrorCode.DELTALAKE_INVALID_METADATA;
        ...
        implements SystemTable
{
    ...
    private static List<Page> buildPages(ConnectorTableMetadata tableMetadata, Table deltalakeTable, long snapshotId)
    {
        PageListBuilder pagesBuilder = PageListBuilder.forTable(tableMetadata);

        Snapshot snapshot = deltalakeTable.snapshot(snapshotId);
        if (snapshot == null) {
            throw new PrestoException(DELTALAKE_INVALID_METADATA, format("Snapshot ID [%s] does not exist for table: %s", snapshotId, deltalakeTable));
        }

        Map<Integer, PartitionSpec> partitionSpecsById = deltalakeTable.specs();
