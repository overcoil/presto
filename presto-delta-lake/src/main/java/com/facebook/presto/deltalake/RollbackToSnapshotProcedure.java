...
import com.facebook.presto.deltalake.DeltaLakeMetadataFactory;
        ...
        implements Provider<Procedure>
{
    ...
    @Inject
    public RollbackToSnapshotProcedure(DeltaLakeMetadataFactory metadataFactory, HdfsEnvironment hdfsEnvironment)
    {
    ...
    public void rollbackToSnapshot(ConnectorSession clientSession, String schema, String table, Long snapshotId)
    {
        ...
        DeltaLakeMetadata metadata = metadataFactory.create();
        ExtendedHiveMetastore metastore = metadata.getMetastore();
        Table deltalakeTable = getDeltaLakeTable(metastore, hdfsEnvironment, clientSession, schemaTableName);
        deltalakeTable.rollback().toSnapshotId(snapshotId).commit();
    }
}
