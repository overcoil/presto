        ...
        implements SystemTable
{
    ...
    private final Table deltalakeTable;

    public SnapshotsTable(SchemaTableName tableName, TypeManager typeManager, Table deltalakeTable)
    {
        requireNonNull(typeManager, "typeManager is null");

        this.deltalakeTable = requireNonNull(deltalakeTable, "deltalakeTable is null");
    ...
    {
        return new FixedPageSource(buildPages(tableMetadata, session, deltalakeTable));
    }

    private static List<Page> buildPages(ConnectorTableMetadata tableMetadata, ConnectorSession session, Table deltalakeTable)
    {
        PageListBuilder pagesBuilder = PageListBuilder.forTable(tableMetadata);

        deltalakeTable.snapshots().forEach(snapshot -> {
